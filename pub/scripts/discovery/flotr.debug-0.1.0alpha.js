...
return {
	...
	draw: function(target, data, options){	
	    ...
	    // Adding Pie chart support

	    function PolarCoordinates(r,theta){
		...
		this.line = [[_x1,_y1],[_x2,_y2]];
	    }

	    function getTheArc(){
		var arc = new Array();
		var points = 0 ;
		var anglePace = ( this.angleEnd - this.angleStart )/ (this.nbOfPoints);
		for(var angle=this.angleStart; angle < this.angleEnd; angle = angle + anglePace){
		    p = new PolarCoordinates(this.radius,angle);
		    arc[points++] = [p.x,p.y];
		}
		return arc ;
	    }

	    function Arc(_angleStart,_angleEnd,_radius , _nbOfPoints){
		this.angleStart=_angleStart;
		this.angleEnd=_angleEnd;
		this.radius=_radius;
		this.nbOfPoints=_nbOfPoints;
		this.arc = getTheArc;
	    }

	    function getLastRadiusLine(){
		var p = new PolarCoordinates(this.radius,this.thetaAbsolute);
		var l= new Line(p.x,p.y,this.xCenter,this.yCenter);
		return l.line;}

	    function getFirstRadiusLine(){
		var p = new PolarCoordinates(this.radius,this.angleStart);
		var l= new Line(this.xCenter,this.yCenter,p.x,p.y);
		return l.line;}

	    function getArc(){
		oneArc = new Arc(this.angleStart,this.thetaAbsolute,this.radius,this.nbOfPoints);
		return oneArc.arc() ;
	    }

	    function CircleSector(_angleStart,_thetaAbsolute,_radius,_nbOfPoints, _xCenter , _yCenter){
		this.angleStart = _angleStart || 0;
		this.radius = _radius || 100;
		this.nbOfPoints = _nbOfPoints || 300;
		this.xCenter = _xCenter || 0;
		this.yCenter = _yCenter || 0;
		this.thetaAbsolute = _thetaAbsolute|| 360;
		this.arc = getArc;
		this.radiusLine1 =getFirstRadiusLine;
		this.radiusLine2 =getLastRadiusLine;
	    }

	    function getSliceOfPie(number){
		var slice = new CircleSector(this.endAngles[number-1],this.endAngles[number],this.radius,50,0,0);
		arc = new Array();
		arc =arc.concat(slice.radiusLine1(),slice.arc(),slice.radiusLine2());
		return arc;
	    }

	    function PieChart(){/* (radius, percentage1, percentage2…) */
		var n = arguments.length;
		this.radius = arguments[0] ;
		this.endAngles =new Array();
		this.endAngles[0] = 0 ;
		for (i=1; i<n; i++)
		    this.endAngles[i] = arguments[i] * 360 + this.endAngles[i-1];
		this.pie= getSliceOfPie;
	    }
