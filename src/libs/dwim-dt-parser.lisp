...
(defun am-pm (string)
  (let ((am-pm (locale-am-pm)))
    (and (simple-string-p string)
         (cond ((string-equal string (car am-pm)) 'am)
               ((string-equal string (cadr am-pm)) 'pm)
               ((string-equal string "am") 'am)
               ((string-equal string "pm") 'pm)
               (t nil)))))
...
(defun weekday (string)
  (when (stringp string)
    (let ((pos (or (position string (locale-day) :test 'string-equal)
                   (position string (locale-abday) :test 'string-equal))))
      (when pos
        (decf pos)
        (if (< pos 0)
            (incf pos 7)
...
(defun month (thing)
  (or (and (stringp thing) 
           (let ((pos (or (position thing (locale-mon) :test 'string-equal)
                          (position thing (locale-abmon) :test 'string-equal))))
             (when pos
               (1+ pos))))
...
(defun get-matching-pattern (patterns string-parts parts-length)
  (when (default-patterns-p patterns)
